...
Usage example::
    ...
    from file_download import download, HashTracker, ProgressTracker

    hasher = HashTracker(hashlib.sha256())
    progress = ProgressTracker(DataTransferBar()

    download('https://github.com/happyshi0402/file_download/file_download/master.zip',
             'file_download.zip', trackers=(hasher, progress))
...
def download(s, url, target, headers=None, trackers=()):
    ...
    # if headers is None:
    #     headers = {}
    # headers.setdefault('user-agent', 'file_download/'+__version__)
